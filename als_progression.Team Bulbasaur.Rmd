...
test_k <- function(trn, tst, k) {
  ...
  for (i in k) {
    pred = FNN::knn.reg(train = as.data.frame(trn[-1]), 
               ...
               as.data.frame(trn[-1]), 
               k     = k[i])
    err[i] <- sqrt(mean(trn$dFRS - pred$pred))
  }
...
lasso.mod = glmnet(as.matrix(als_train[,-1]), als_train$dFRS,  alpha=1, lambda=grid)
...
cv.out=cv.glmnet(as.matrix(als_train[,-1]), als_train$dFRS,  alpha=1)
...
cv.out = cv.glmnet(as.matrix(trainfold[,-1]), trainfold$dFRS,  alpha=1)
bestlam=cv.out$lambda.min

  # We predict on the test fold
predicted <- predict(lasso.mod,s=bestlam,as.matrix(testfold[-1]))
