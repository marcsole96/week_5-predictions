...
cross_validation <- function(dataset, k=10){
  ...
  for (i in 1:k){                                                 # For each fold
    ...
    fit       <- lm(response~., data=traindata)                       # Fit the model with training data
  ...
  rmse_cv <- sqrt(mean((observed-predicted)^2))        # Calculate CV RMSE
    ...
    fit        <- lm(response~., data=dataset)                          # Fit the model with all the dataset
  observed   <- dataset$response                                      # Get observed values
...
regfit.fwd=regsubsets(response~.,data=trainfold ,nvmax =30,
method ="forward")
coeficients=names(coef(regfit.fwd,30))
myForm <- as.formula(paste("response ~ ", paste (coeficients[-1], collapse=" + "), sep=""))

mlg <- lm(myForm,data=trainfold)
...
lasso.mod = glmnet(as.matrix(trainfold[,-1]), trainfold$response,  alpha=1)
...
allpreds.test <- cbind(pMLG, pLASSO, pBoost, pRF)
...
>Prediction of the full data using means

```{r Model averaging , eval=FALSE, include=FALSE}
#Lasso
cv.out=cv.glmnet(as.matrix(als_train[,-1]), als_train$response,  alpha=1)
bestlam=cv.out$lambda.min

pLASSO=predict(lasso.mod,s=bestlam,as.matrix(als_predict[-1]))
colnames(pLASSO) <- "pLASSO"

#MLG
regfit.fwd=regsubsets(response~.,data=als_train ,nvmax =30,
method ="forward")
coeficients=names(coef(regfit.fwd,30))
myForm <- as.formula(paste("response ~ ", paste (coeficients[-1], collapse=" + "), sep=""))

mlg <- lm(myForm,data=als_train)
...
pRF <- randomForest(response ~., data=als_train, ntree=500, mtry = 200, importance=TRUE, do.trace=TRUE)
pRF <- predict(object=pRF, newdata = als_predict)


allpreds.test <- cbind(pMLG, pLASSO, pRF)
submission <- rowMeans(allpreds.test)
```

>Prediction of the full data using just RF

```{r}
#Run The top CV before

mse <- mean((observed-pRF)^2)
...
submission <- pRF
submission=data.frame(predicted=submission)
```
